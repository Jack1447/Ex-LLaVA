...
def http_bot(state, model_selector, temperature, top_p, max_new_tokens, request: gr.Request):
    ...
    # 如果前端下拉为空，尝试动态刷新一次模型列表并选择第一个可用模型
    if not model_name:
        try:
            available_models = get_model_list()
            if len(available_models) > 0:
                model_name = available_models[0]
                logger.info(f"No model selected, fallback to first available model: {model_name}")
        except Exception as e:
            logger.info(f"Failed to refresh model list: {e}")
    ...
    if len(state.messages) == state.offset + 2:
        ...
        # 不再强制覆盖为 mistral_instruct，避免与训练不一致
    ...
    try:
        ...
        response = requests.post(worker_addr + "/worker_generate_stream",
            headers=headers, json=pload, stream=True, timeout=300)
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Accordion("Parameters", open=False) as parameter_row:
                    ...
                    max_output_tokens = gr.Slider(minimum=0, maximum=16384, value=2048, step=64, interactive=True, label="Max output tokens",)
...
if __name__ == "__main__":
    ...
    # 支持 --controller 的别名，兼容你的启动命令
    parser.add_argument("--controller-url", "--controller", type=str, default="http://localhost:21001")
    parser.add_argument("--concurrency-count", type=int, default=10)
    # 默认改为 reload，保证在先开 gradio 后开 worker 的情况下能刷新到模型
